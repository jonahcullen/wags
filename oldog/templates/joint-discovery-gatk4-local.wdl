...
#  File eval_interval_list
  ...
  File dbsnp146_snp_vcf
  File dbsnp146_snp_vcf_index
  File dbsnp146_indels_vcf
  File dbsnp146_indels_vcf_index
  File broad_snp_vcf
  File broad_snp_vcf_index
  File axelsson_snp_vcf
  File axelsson_snp_vcf_index
  File illumina_snp_vcf
  File illumina_snp_vcf_index
...
#  Int num_gvcfs = length(input_gvcf)
  Int num_gvcfs = 1
#
...
#  scatter (interval in scattered_calling_intervals) {
...
#        interval = unpadded_intervals[idx],
        ...
        ref_dict = ref_dict,
#        dbsnp_vcf = dbsnp146_snp_vcf,
#        dbsnp_vcf_index = dbsnp146_snp_vcf_index,
#        disk_size = medium_disk,
#        docker = gatk_docker,
#        gatk_path = gatk_path
        gatk = gatk
...
#      dbsnp_resource_vcf_index = dbsnp_resource_vcf_index,
      dbsnp146_indels_vcf = dbsnp146_indels_vcf,
      dbsnp146_indels_vcf_index = dbsnp146_indels_vcf_index,
...
# Do not need the following - yet - but do not delete
#  if (num_gvcfs > 10000) {
...
#          dbsnp_resource_vcf = dbsnp_resource_vcf,
#          dbsnp_resource_vcf_index = dbsnp_resource_vcf_index,
          dbsnp146_snp_vcf = dbsnp146_snp_vcf,
          dbsnp146_snp_vcf_index = dbsnp146_snp_vcf_index,
          ...
          illumina_snp_vcf = illumina_snp_vcf,
          illumina_snp_vcf_index = illumina_snp_vcf_index,        
#          disk_size = small_disk,
#          docker = gatk_docker,
#          gatk_path = gatk_path
          gatk = gatk
...
#        snps_recalibration = if defined(SNPsVariantRecalibratorScattered.recalibration) then select_first([SNPsVariantRecalibratorScattered.recalibration])[idx] else select_first([SNPsVariantRecalibratorClassic.recalibration]),
        snps_recalibration = select_first([SNPsVariantRecalibratorClassic.recalibration]),
#        snps_recalibration_index = if defined(SNPsVariantRecalibratorScattered.recalibration_index) then select_first([SNPsVariantRecalibratorScattered.recalibration_index])[idx] else select_first([SNPsVariantRecalibratorClassic.recalibration_index]),
        snps_recalibration_index = select_first([SNPsVariantRecalibratorClassic.recalibration_index]),
...
#        gatk_path = gatk_path
  ...
  call VcfMeetVepAndTabix {
    input:
      input_vcf = FinalGatherVcf.output_vcf,
      input_vcf_index = FinalGatherVcf.output_vcf_index,
#      cache = current_cache,
      out_name = callset_name + ".vep.vcf"
  }
...
#  }
  ...
  output {
   # outputs from the small callset path through the wdl
    ...
    Array[String] variant_filtered_vcf = HardFilterAndMakeSitesOnlyVcf.variant_filtered_vcf
    Array[String] sites_only_vcf = HardFilterAndMakeSitesOnlyVcf.sites_only_vcf

    File final_annotated_vcf = VcfMeetVepAndTabix.annotated_vcf
...
# NOTE: --only-output-calls-starting-in-intervals not available in GATK3
# and thus variants that land at intervals will be doubled...

# NOTE: --useNewAFCalculator causes the --max_alternate_alleles to be ignored
# resulting in up to 49 alternate alleles in the VCF as of 20200115

  command <<<
    set -e

    ${gatk} --java-options "-Xmx28g -Xms5g" \
      GenotypeGVCFs \
      -R ${ref_fasta} \
...
#  String walltime
  ...
  command {
    ${gatk} --java-options "-Xmx24g -Xms24g" \
      ...
      --rscript-file INDEL_recal.plots.R \
      --trust-all-polymorphic \
      -tranche ${sep=' -tranche ' recalibration_tranche_values} \
      -an ${sep=' -an ' recalibration_annotation_values} \
      -mode INDEL \
      --max-gaussians 4 \
      --resource:dbsnp,known=false,training=true,truth=true,prior=10 ${dbsnp146_indels_vcf}
  }
#  runtime {
#    docker: docker
#    memory: "26 GB"
...
#  Int disk_size
  ...
  command {
    ${gatk} --java-options "-Xmx12g -Xms3g" \
      ...
      --max-gaussians 6 \
      --resource:illumina,known=true,training=true,truth=true,prior=15.0 ${illumina_snp_vcf} \
      --resource:broad,known=true,training=true,truth=true,prior=10.0 ${broad_snp_vcf} \
      --resource:axelsson,known=false,training=true,truth=true,prior=8.0 ${axelsson_snp_vcf} \
      --resource:dbSNP146,known=true,training=true,truth=true,prior=12.0 ${dbsnp146_snp_vcf}
  }
#  runtime {
#    docker: docker
#    memory: "3.5 GB"
...
#  String cache
  ...
  command {
    ...
    bgzip -c ${out_name} > ${out_name}.gz &&
    tabix -p vcf ${out_name}.gz
  }
  output {
    File annotated_vcf = "${out_name}.gz" 
  }
}
