...
#  File unpadded_intervals_file
  ...
  File scattered_db_intervals_list
...
#  }
  ...
  Array[File] scattered_intervals = read_lines(scattered_db_intervals_list)
#  Array[String] unpadded_intervals = read_lines(DynamicallyCombineIntervals.output_intervals)
  scatter (idx in range(length(scattered_intervals))) {
...
#  scatter (idx in range(length(unpadded_intervals))) {
    ...
    call ImportGVCFs {
      input:
        ...
        interval = scattered_intervals[idx],
        workspace_dir_name = basename(scattered_intervals[idx], ".interval_list"),
        input_gvcfs = input_gvcfs,
        input_gvcfs_indices = input_gvcfs_indices,
        batch_size = 50,
        tmp_dir = "/dev/shm/" + basename(scattered_intervals[idx]),
        gatk = gatk
    }
...
#  }
  ...
  output {
    ...
    Array[String] output_db = ImportGVCFs.output_genomicsdb
