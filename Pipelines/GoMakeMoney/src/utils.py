...
def sequence_grouping(ref_dict,ref):
    '''
    Placeholder
    '''
    ...
    if not os.path.isdir(f"results/seq_group/{ref}/no_unmap"):
        os.makedirs(f"results/seq_group/{ref}/no_unmap",exist_ok=True)
        for i,v in enumerate(seq_group):
            with open(os.path.join(f"results/seq_group/{ref}/no_unmap",f"group_{i}.tsv"),"w") as f:
                print(v,file=f)
    ...
    if not os.path.isdir(f"results/seq_group/{ref}/with_unmap"):
        os.makedirs(f"results/seq_group/{ref}/with_unmap",exist_ok=True)
        for i,v in enumerate(seq_group_unmapped):
            with open(os.path.join(f"results/seq_group/{ref}/with_unmap",f"group_{i}.tsv"),"w") as f:
                print(v,file=f)
