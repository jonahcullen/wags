...
rule indels_var_recal:
    ...
    output:
        ...
        indels_plot     = "{bucket}/wgs/{breed}/{sample_name}/{ref}/money/var_recal/indels.plot.R",
        indels_plot_pdf = S3.remote("{bucket}/wgs/{breed}/{sample_name}/{ref}/money/var_recal/indels.plot.R.pdf")
    params:
        ...
        dbsnp_indels_vcf  = config['known_sites']['dbsnp_indels_vcf']
       #sites     = " --known-sites ".join(list(config['known_sites'].values())) # THIS IS FROM bam.smk
    threads: 4
    resources:
         time   = 240,
         mem_mb = 24000
...
rule snps_var_recal:
    ...
    output:
        ...
        snps_plot     = "{bucket}/wgs/{breed}/{sample_name}/{ref}/money/var_recal/snps.plot.R",
        snps_plot_pdf = S3.remote("{bucket}/wgs/{breed}/{sample_name}/{ref}/money/var_recal/snps.plot.R.pdf"),
        tranches_pdf  = S3.remote("{bucket}/wgs/{breed}/{sample_name}/{ref}/money/var_recal/snps.tranches.pdf"),
    params:
        ...
        broad_snp_vcf     = config['known_sites']['broad_snp_vcf'],
        axelsson_snp_vcf  = config['known_sites']['axelsson_snp_vcf'],
        illumina_snp_vcf  = config['known_sites']['illumina_snp_vcf']
    threads: 4
    resources:
         time   = 240,
         mem_mb = 24000
