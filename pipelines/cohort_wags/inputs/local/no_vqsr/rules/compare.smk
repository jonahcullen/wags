...
rule select_variants_to_table:
    input:            
        final_vcf  = "{bucket}/wgs/pipeline/{ref}/{date}/final_gather/vep/vep_{vep_interval}/joint_call.{vep_interval}.vep.vcf.gz", 
    ...
    output:
        unique_vars           = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/{vep_interval}/joint_money.{ref}.{vep_interval}.unique_vars.vcf",
        rare_and_common_vars  = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/{vep_interval}/joint_money.{ref}.{vep_interval}.rare_and_common_vars.vcf",
        rare_vars             = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/{vep_interval}/joint_money.{ref}.{vep_interval}.rare_vars.vcf",
        unique_vars_vep_split = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/{vep_interval}/joint_money.{ref}.{vep_interval}.unique_vars.vep_split.txt",
        rare_vars_vep_split   = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/{vep_interval}/joint_money.{ref}.{vep_interval}.rare_vars.vep_split.txt"
    params:
...
def get_uniq_vcfs(wildcards):
    ...
    return expand(
        os.path.join(
            comp_dir,
            "joint_money.{ref}.{vep_interval}.unique_vars.vcf"
        ),bucket=config["bucket"],ref=config['ref'],vep_interval=SPLIT,date=config['date']
    )
...
rule merge_uniq_vcfs:
    input:
        get_uniq_vcfs
    output:
        vcf = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/joint_money.{ref}.unique_vars.vcf.gz",
        tbi = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/joint_money.{ref}.unique_vars.vcf.gz.tbi",
    params:
    ...
    return expand(
        os.path.join(
           ...
           #"{breed}_{sample_name}.{ref}.{split}.rare_vars.vcf"
            "joint_money.{ref}.{vep_interval}.rare_vars.vcf",
        ),bucket=config["bucket"],ref=config['ref'],vep_interval=SPLIT,date=config['date']
    )
...
rule merge_rare_vcfs:
    input:
        get_rare_vcfs
    output:
        vcf = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/joint_money.{ref}.rare_vars.vcf.gz",
        tbi = "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/joint_money.{ref}.rare_vars.vcf.gz.tbi",
    params:
        vcfs     = lambda wildcards, input: " -INPUT ".join(map(str,input)),
        java_opt  = "-Xmx2000m",
    threads: 1
    ...
    return expand(
        os.path.join(
           ...
           #"{breed}_{sample_name}.{ref}.{split}.unique_vars.vep_split.txt"
            "joint_money.{ref}.{vep_interval}.unique_vars.vep_split.txt",
        ),
...
rule merge_uniq_txts:
    input:
        get_uniq_txts
    output:
        "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/joint_money.{ref}.unique_vars.vep_split.txt"
    threads: 1
    resources:
         time   = 120,
         mem_mb = 4000
    ...
    return expand(
        os.path.join(
           ...
           #"{breed}_{sample_name}.{ref}.{split}.rare_vars.vep_split.txt"
            "joint_money.{ref}.{vep_interval}.rare_vars.vep_split.txt"
        ),
        bucket=config["bucket"],
        ref=config['ref'],
        vep_interval=SPLIT,
...
rule merge_rare_txts:
    input:
        get_rare_txts
    output:
        "{bucket}/compare_pop/select_vars_to_table/{ref}/{date}/joint_money.{ref}.rare_vars.vep_split.txt"
    threads: 1
    resources:
         time   = 120,
         mem_mb = 4000
