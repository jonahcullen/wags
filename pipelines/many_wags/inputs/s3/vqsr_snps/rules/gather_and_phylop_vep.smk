    ...
    shell:
        '''
            gatk SplitIntervals \
                ...
                -O {output}
        '''
# /scratch.global/hamsh010/3kbuild/joint_call/friedlab/wgs/pipeline/UU_Cfam_GSD_1.0_ROSY/20241122/final_gather/joint_call.UU_Cfam_GSD_1.0_ROSY.20241122.vep.vcf.gz
...
rule final_gather_veps:
    input:
        get_vep_vcfs
    output:
        vep_vcf = S3.remote("{bucket}/wgs/pipeline/{ref}/{date}/final_gather/joint_call.{ref}.{date}.vep_exact.vcf.gz",keep_local=True),
    params:
...
rule tabix_vep_vcf:
    ...
    output:
        S3.remote("{bucket}/wgs/pipeline/{ref}/{date}/final_gather/joint_call.{ref}.{date}.vep_exact.vcf.gz.tbi",keep_local=True),
    threads: 12
    resources:
         time   = 5000,
         mem_mb = 22000
    shell:
        '''
            tabix -p vcf {input.vep_vcf}
        '''
           #bgzip --threads {threads} -c {input.vep_vcf} > {output.vep_vcf} &&
...
rule gbindex_vep_vcf:
    ...
    output:
        S3.remote("{bucket}/wgs/pipeline/{ref}/{date}/final_gather/joint_call.{ref}.{date}.vep_exact.vcf.gz.covtsf"),
    params:
