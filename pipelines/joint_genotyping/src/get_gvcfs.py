...
# get dog ids 
d = defaultdict(list)
with open(config["current_dogids"]) as f:
    dogids = [d[line.rstrip()] for line in f]
...
if not os.path.isfile("gvcfs.list"):
    ...
    for i in objects[1:]:
        ...
        breed,dogid = i.split("/")[3:5]
        if (dogid in d) and (i.endswith(".g.vcf.gz")) and (config['ref'] in i):
            d[dogid].extend([breed,i])

    with open("gvcfs.list","w") as f:
        f.write("dogid\tbreed\tgvcf\n")
        for k,v in d.items():
            f.write(f"{k}\t{v[0]}\t{v[1]}\n")
