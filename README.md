...
## Individual SV VCFs to population level VCF (ManySVs)

**Prepare joint SV calling inputs**

To prepare for the joint SV `many_svs` genoptying pipeline, you must copy all desired CRAMs generated by one_wag into a new directory and individual SV vcfs into a separate directory with sub directories named delly, gridss, smoove and manta. i.e.
...
Next, `prep_svs.py` is used to prepare the configuration file and submission script. If the species and reference of interest is already within the container (e.g. dog - canfam4), `--fasta`, `--fasta_dict`, and `--gtf` do not need to be set as optional arguments. An example of preparing the configuration file for canfam4 is shown below (see `python prep_svs.py --help` for all available arugments)

To setup the pipeline inputs, `prep_svs.py` has the following options

```
required arguments:
  -r, --ref         select reference to use:
                      canfam3, canfam4, UU_Cfam_GSD_1.0_ROSY, goldenPath, Arabian, Shire, Thoroughbred, tiger, Fca126_mat1.0, alpaca, ARS-UI_Ramb_v3.0, ARS1.2
  -c, --crams       path to cram directory
  -sv, --svars      path to sv directory
  -b, --bucket      results directory or bucket name if using
                      --remote s3
  -s, --snake-env   conda environment with snakemake
  -i, --sif         path to singularity image file
  -p, --partition   default partition(s) to use (e.g. 'par1' or 'par1,par2'
  -e, --email       email address for job logs
  -a, --account     default scheduler account
  -o, --out         path to out dir
...
optional arguments:
  --walltime WALLTIME   wall time (hours) for main job runner
                        [default: 48]
  --sv-overlap-size SV_OVERLAP_SIZE
  ...
  --sv-length SV_LENGTH
                        minimum length needed to keep an SV [default: 50]
  --profile PROFILE     HPC job scheduler [default: slurm]
  --remote REMOTE       save outputs to remote: S3, SFTP [default: local]
  --alias ALIAS         minio client S3 storage alias [default: s3]
  -h, --help            show this help message and exit
```

where `--crams` is the full path to the newly generated `crams` directory, and `--svars` is the full path to the top level `svars` directory containing the sv vcfs. `--sv-overlap` is maximum distance (in bp) two predicted SVs can be to be merged into 1 SV by `SURVIVOR`, `--sv-callers` defines how many SV predictors are needed to keep and genotype an SV, and `--sv-length` defines the minimum length (in bp) of an SV to be retained and genotyped.

NOTE that the `many_svs` pipeline will only retain insertions (INS), deletions (DEL), duplications (DUP), and inversions (INV). Break-end (BND) SVs are removed given that most are long unresolved or complex SVs. 

`many_svs` works by first merging SV predictions among individuals for each caller (i.e. `delly`, `manta`, `gridss`, `smoove`). SV calls from each predictor are then merged together with `SURVIVOR`. `many_svs` retains **all insertions** predicted by `manta` given other tools offer none or less robust insertion predictions. By default, DUPs, DELs, and INVs require predictions by two callers to be retained and genotyped. SV genotyping is performed by `graphtyper`, and SV impacts are predicted by `VEP`.